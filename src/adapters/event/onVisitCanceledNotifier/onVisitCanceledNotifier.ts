...
import { visitCanceledNotifierHandler } from "../../../app/event/visitCanceledNotifierHandler/visitCanceledNotifierHandler";
...
> = asyncMiddleware(async (event) => {
  ...
  await visitCanceledNotifierHandler({ visitEventStream }, { notifiers });
});
