...
const acceptVisitInvitationCommandMock = jest.fn();

jest.mock(
  "../../../app/commands/acceptVisitInvitationCommand/acceptVisitInvitationCommand",
  () => ({
    acceptVisitInvitationCommand: acceptVisitInvitationCommandMock
  })
);

// WARNING: This import must be at the end
// eslint-disable-next-line import/first
import { handler } from "./acceptVisitInvitation";

describe("acceptVisitInvitation", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  test("should call createVisitCommand with the correct parameters and return ID on success", async () => {
    ...
    // assert
    expect(acceptVisitInvitationCommandMock).toHaveBeenCalledWith(expectedInput, expectedContext);
  });
});
