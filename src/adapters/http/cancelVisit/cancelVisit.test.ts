...
const cancelVisitCommandMock = jest.fn();

jest.mock("../../../app/commands/cancelVisitCommand/cancelVisitCommand", () => ({
  cancelVisitCommand: cancelVisitCommandMock
}));

// WARNING: This import must be at the end
// eslint-disable-next-line import/first
import { handler } from "./cancelVisit";
...
describe("cancelVisit", () => {
  ...
  test("should call createVisitCommand with the correct privileges and admin access", async () => {
    // arrange
    ...
    // assert
    expect(cancelVisitCommandMock).toHaveBeenCalledWith(expectedInput, expectedContext);
  });

  test("should call createVisitCommand with the correct privileges and no admin access", async () => {
    // arrange
