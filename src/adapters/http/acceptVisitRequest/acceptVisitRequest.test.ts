...
const acceptVisitRequestCommandMock = jest.fn();

jest.mock("../../../app/commands/acceptVisitRequestCommand/acceptVisitRequestCommand", () => ({
  acceptVisitRequestCommand: acceptVisitRequestCommandMock
}));

// WARNING: This import must be at the end
// eslint-disable-next-line import/first
import { handler } from "./acceptVisitRequest";
...
describe("acceptVisitRequest", () => {
  ...
  test("should call acceptVisitRequestCommand with the correct privileges and admin access", async () => {
    ...
    // assert
    expect(acceptVisitRequestCommandMock).toHaveBeenCalledWith(expectedInput, expectedContext);
  });
